...
export function FetchMobileAnalyticsSummary() {
...
export function FetchMobileAnalyticsTransactions(limit: number) {
...
export const MobileAnalyticsView: FC = () => {
    const { summary } = FetchMobileAnalyticsSummary();
    const { transactions } = FetchMobileAnalyticsTransactions(10);

    const pctMembership = summary
        ? (summary.revenue_by_source.membership_fees /
              (summary.revenue_by_source.membership_fees +
                  summary.revenue_by_source.personal_training || 1)) *
          100
        : 0;
    const pctPT = summary
        ? (summary.revenue_by_source.personal_training /
              (summary.revenue_by_source.membership_fees +
                  summary.revenue_by_source.personal_training || 1)) *
          100
        : 0;
    const totalSource = summary
        ? summary.revenue_by_source.membership_fees +
          summary.revenue_by_source.personal_training
        : 0;
        ...
        <section className="page active" aria-labelledby="page-title">
            <article className="summary-card">
                ...
                <p className="summary-value mono">
                    ₹{summary?.total_income_ytd.toLocaleString() || 0}
                </p>
                <p className="summary-copy">
                    Membership subscriptions remain the main engine, while
                    training keeps margin quality high.
                </p>
            ...
            <article className="card">
                <div className="section-head">
                    ...
                    <p className="section-label mono">
                        ₹{totalSource.toLocaleString()} total
                    </p>
                </div>
                        ...
                        >
                            <div className="transaction-topline">
                                <div>
                                    ...
                                    <p className="transaction-meta mono">
                                        TX-{tx.transaction_id} ·{" "}
                                        {new Date(tx.date).toLocaleDateString()}
                                    </p>
                                </div>
                                <span
                                    className={`status-pill ${tx.status === "paid" ? "completed" : "pending"}`}
                                >
                                    {tx.status}
                                </span>
                            </div>
                            <div className="stateline">
                                ...
                                <div style={{ textAlign: "right" }}>
                                    <span className="mini-meta">Plan</span>
